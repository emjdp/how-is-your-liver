...
5. 사용자 사진 배경 카드 **(v1.0 필수)**
   - 사용자가 업로드한 사진을 카드 배경으로 사용
   - 업로드 즉시 다운스케일 후 dataURL만 보관. 원본 대용량 dataURL 장기 보관 금지
   - 50~65% 어두운 오버레이. 텍스트 가독성 WCAG AA 유지
   - 서버 전송 없음. 기기 안에서만 처리
   - `html-to-image` 캡처 안정성 검증 필수
   - 우선 `tpl_report`, `tpl_overtime`에만 적용 (tpl_forecast, tpl_warning은 v1.1 검토)
...
2. 실제 실행 순서는 `docs/V1_TODO.md`를 단일 기준으로 따른다.
3. 한 번에 너무 넓게 건드리지 말고 단계별로 구현해라.
4. 각 단계마다 테스트를 추가하거나 기존 테스트를 갱신해라.
5. 기존 기능이 깨지지 않게 npm test / npm run lint / npm run build를 계속 확인해라.
6. 스토리 카드 관련 변경은 실제 PNG 생성 검증을 포함해라.
...
9단계 — 사용자 사진 배경 카드 **(v1.0 필수)**
- v1.0 필수로 진행한다. (V1_TODO.md Step 10과 동일)
- 사진은 다운스케일한 dataURL로만 처리하고 서버 전송 금지.
- 업로드 즉시 다운스케일. 원본 대용량 dataURL 장기 보관 금지.
- 50~65% 어두운 오버레이로 텍스트 가독성을 보장한다.
- 우선 tpl_report, tpl_overtime에만 적용.
