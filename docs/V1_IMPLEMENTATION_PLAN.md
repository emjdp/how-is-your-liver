...
- 메인 화면 상단 우측 또는 요일바 근처의 작은 아이콘 버튼
- 결과/카드 페이지에도 접근 가능해야 한다.
...
- 마스코트 파일을 `public/brand/`에 배치한다.
- favicon/app icon/OG 이미지를 적용한다.
- `app/layout.tsx` metadata/icons/openGraph를 업데이트한다.
- 마스코트는 메인/빈 상태/결과/카드에 절제해서 적용한다.

완료 기준:

- favicon이 브라우저 탭에 표시된다.
- 모바일 홈 화면 아이콘 후보가 준비된다.
- OG 이미지가 1200×630으로 존재한다.
- 마스코트가 UI를 술게임처럼 보이게 만들지 않는다.
...
1. 하이볼 기준 ABV를 7.0%로 고정해도 되는가?
2. 소주 잔 입력은 병 입력과 동시에 허용할 것인가?
3. 마스코트는 어떤 화면에 먼저 넣을 것인가?
4. 다크모드 토글 아이콘 위치는 메인 상단 우측으로 할 것인가?
5. JSON export만 v1.0에 넣고 import는 제외해도 되는가?
6. 사용자 사진 배경 카드는 v1.0 필수인가, v1.1로 미룰 것인가?

## 7. 권장 커밋 단위

1. `docs: plan v1 expansion`
2. `feat: support soju glass and highball calculations`
3. `feat: expand drink input UI`
4. `feat: add theme mode toggle`
5. `feat: add brand assets and metadata`
6. `feat: add expanded story card templates`
7. `feat: add weekly story cards`
8. `feat: export records as json`
9. `feat: add custom photo story card background`
