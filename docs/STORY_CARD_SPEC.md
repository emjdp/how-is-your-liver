...
### v1.0 추가 (3종)
| ID | 이름 | 콘셉트 | 강조 수치 | 대상 | 라우트 |
|---|---|---|---|---|---|
| `tpl_forecast` | 숙취 예보 카드 | 일기예보 패러디 | 처리 추정 시간 + 한줄 예보 | 당일 | `/result/card` |
| `tpl_warning` | 블랙코미디 경고 카드 | 안전 표지판 패러디 (지나치게 멋있어 보이지 않게 채도·밝기 낮춘 버전으로 절제) | 티어 카피 메인 | 당일 | `/result/card` |
| `tpl_weekly` | 주간 정산 카드 | 회계 정산서 패러디 | 최근 7일 알코올 g + 음주일 수 | 주간 | `/weekly/card` |

**당일 카드**: `tpl_report`, `tpl_overtime`, `tpl_forecast`, `tpl_warning` (4종, `/result/card`).  
**주간 카드**: `tpl_weekly` (1종, `/weekly/card`, v1.0 포함).

모든 카드 PNG는 **1080×1920** 유지.
...
## 사용자 사진 배경 카드 (v1.0 포함)

사용자가 업로드한 사진을 당일 카드 일부 템플릿의 배경으로 사용하는 기능.

### 적용 범위 (v1.0)
- 우선 `tpl_report`, `tpl_overtime`에 적용. `tpl_forecast`, `tpl_warning`은 v1.1 검토.

### 데이터 처리
- 업로드 즉시 canvas 또는 동등한 방식으로 다운스케일.
  - 권장 출력: 1080×1920 cover crop 또는 긴 변 2160px 이하.
- 저장 상태에는 다운스케일된 dataURL만 보관. 원본 대용량 dataURL 장기 보관 금지.
- 서버 전송 없음. 사진은 기기 안에서만 처리.
- 10MB 초과 파일 차단.

### 렌더링
- `<img object-fit: cover>` + **50~65% 어두운 오버레이** (`linear-gradient(rgba(11,20,17,0.50..0.65), ...)` 적용).
- 텍스트는 세이프 영역에 흰색 + 가벼운 drop-shadow.
- 텍스트 가독성 WCAG AA 수준 유지.
- `html-to-image` 캡처 시 cross-origin 오류 방지: dataURL로 변환 후 캔버스에 그린다.
